...
public:
    NotificationsMock(std::vector<std::string> capabilities = {
                          "actions", "body", "body-markup", "icon-static", "image/svg+xml",
                          "x-canonical-private-synchronous", "x-canonical-append", "x-canonical-private-icon-only",
                          "x-canonical-truncation", "private-synchronous", "append", "private-icon-only", "truncation"})
    {
        ...
        baseobj = dbus_test_dbus_mock_get_object(mock, "/org/freedesktop/Notifications",
                                                 "org.freedesktop.Notifications", nullptr);

        std::string capspython("ret = ");
        capspython += vector2py(capabilities);
        dbus_test_dbus_mock_object_add_method(mock, baseobj, "GetCapabilities", nullptr, G_VARIANT_TYPE("as"),
                                              capspython.c_str(), nullptr);

        dbus_test_dbus_mock_object_add_method(mock, baseobj, "GetServerInformation", nullptr, G_VARIANT_TYPE("(ssss)"),
                                              "ret = ['notification-mock', 'Testing harness', '1.0', '1.1']", nullptr);

        dbus_test_dbus_mock_object_add_method(mock, baseobj, "Notify", G_VARIANT_TYPE("(susssasa{sv}i)"),
                                              G_VARIANT_TYPE("u"), "ret = 10", nullptr);

        dbus_test_dbus_mock_object_add_method(mock, baseobj, "CloseNotification", G_VARIANT_TYPE("u"), nullptr, "",
                                              nullptr);
    }
    ...
    {
        return std::shared_ptr<GVariant>(g_variant_get_child_value(tuple, index), [](GVariant* v) {
            if (v != nullptr)
                g_variant_unref(v);
        });
    }
    ...
    std::vector<Notification> getNotifications(void)
    {
        ...
        for (i = 0; i < cnt; i++)
        {
            ...
            while (g_variant_iter_loop(&ihints, "{&sv}", &hint_key, &hint_value))
            {
                ...
                std::shared_ptr<GVariant> value(g_variant_ref(hint_value), [](GVariant* v) {
                    if (v != nullptr)
                        g_variant_unref(v);
                });
